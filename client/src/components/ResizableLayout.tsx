...
const ResizableLayout: React.FC<ResizableLayoutProps> = ({
  ...
  const handleMouseMove = useCallback((e: MouseEvent) => {
    ...
    // Constrain the width
    newLeftWidth = Math.max(minLeftPercentage, Math.min(100 - minRightPercentage, newLeftWidth));
  ...
  return (
    <div ref={containerRef} className="flex h-full w-full">
      ...
      >
        {/* Drag Handle */}
        <div className={`absolute inset-y-0 flex items-center justify-center w-4 -ml-2 transition-all duration-200 ${
          isDragging ? 'bg-purple-100 border border-purple-300' : 'group-hover:bg-gray-100'
        } rounded-sm`}>
          <GripVertical className={`w-3 h-3 transition-colors duration-200 ${
            isDragging ? 'text-purple-600' : 'text-gray-400 group-hover:text-gray-600'
          }`} />
        </div>

        {/* Visual feedback line */}
        <div className={`absolute inset-y-0 w-0.5 transition-all duration-200 ${
