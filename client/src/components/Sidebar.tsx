...
import { Plus, MessageCircle, Users, TrendingUp, Settings, X, Trash2 } from 'lucide-react';
...
const Sidebar: React.FC<SidebarProps> = ({ isOpen, onClose }) => {
  const { currentDashboard, dashboards, setCurrentDashboard, addDashboard, removeDashboard } = useDashboardContext();
  const { clearMessages } = useChatContext();
  const [showCreateForm, setShowCreateForm] = useState(false);
  const [newDashboardName, setNewDashboardName] = useState('');

  useEffect(() => {
    // Initialize dashboards from JSON on first load
  ...
  return (
    <>
      ...
      {/* Sidebar */}
      <div className={`fixed left-0 top-0 h-full w-80 bg-white shadow-xl z-50 transform transition-transform duration-300 flex flex-col ${
        ...
        <div className="flex-shrink-0 p-6 border-b border-gray-200 bg-gradient-to-r from-purple-50 to-blue-50">
          ...
          <p className="text-sm text-gray-600 mt-1">Manage your analytics dashboards</p>
        </div>
            ...
            return (
              ...
              >
                <div className="flex items-start space-x-3">
                  <div className={`p-2 rounded-lg bg-gradient-to-r ${getColorClasses(dashboard.color)} text-white flex-shrink-0`}>
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs text-gray-500 mt-2">
                      Last accessed: {dashboard.lastAccessed.toLocaleDateString()}
                    </p>
                  </div>
